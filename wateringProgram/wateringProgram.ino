int pumpPin = 7; // pin that turns on the motor
...
void setup() {
  pinMode(pumpPin, OUTPUT); // set 7 to an output so we can use it to turn on the MOSFET
  digitalWrite(pumpPin, LOW); //set 7 to LOW so pump is not powered on start up
  pinMode(moisturePower, OUTPUT); //set 4 as an output
...
int readMoisture() {
  digitalWrite(moisturePower, HIGH); //turn D4 ON
  delay(1000); //wait one second
  moistureValue = analogRead(moisturePin); //Read the SIG (signal) value from the sensor
  digitalWrite(moisturePower, LOW); //turn D4 OFF
